...
textAngular.directive("textAngular", ['$compile', '$window', '$document', '$rootScope', '$timeout', function($compile, $window, $document, $rootScope, $timeout) {
	...
	$rootScope.textAngularTools = deepExtend({
		...
		insertLink: {
			...
			action: function() {
				var urlLink;
				urlLink = prompt("Voeg de URL voor de link toe", 'http://');
	...
	return {
		...
		link: function(scope, element, attrs, ngModel) {
			...
			scope.displayElements = {
				...
				text: angular.element("<div contentEditable='true' class='well well-sm' ng-hide='showHtml' ta-bind='text' ng-model='text' ></div>")
			};
