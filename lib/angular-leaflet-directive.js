...
/*OBS
angular.module("leaflet-directive").directive('d3layer', function ($log, $rootScope, leafletData, leafletHelpers) {
    return {
        restrict: "A",
        scope: false,
        replace: false,
        ...
        link: function(scope, element, attrs, controller) {
            ...
            controller.getMap().then(function(map) {
                leafletScope.$watch("d3layer", function(d3layer) {
                    if (isDefined(leafletGeoJSON) && map.hasLayer(leafletGeoJSON)) {
                        map.removeLayer(leafletGeoJSON);
                    }

                    if (!(isDefined(d3layer) && isDefined(d3layer.data))) {
                        return;
                    }

                    var resetStyleOnMouseout = d3layer.resetStyleOnMouseout,
                        onEachFeature = d3layer.onEachFeature;

                    if (!onEachFeature) {
                        onEachFeature = function(feature, layer) {
                            if (leafletHelpers.LabelPlugin.isLoaded() && isDefined(d3layer.label)) {
                                layer.bindLabel(feature.properties.description);
                            }
                            ...
                            layer.on({
                                mouseover: function(e) {
                                    safeApply(leafletScope, function() {
                                        d3layer.selected = feature;
                                        $rootScope.$broadcast('leafletDirectiveMap.d3layerMouseover', e);
                                    });
                                },
                                mouseout: function(e) {
                                    if (resetStyleOnMouseout) {
                                    ...
                                    safeApply(leafletScope, function() {
                                        d3layer.selected = undefined;
                                        $rootScope.$broadcast('leafletDirectiveMap.d3layerMouseout', e);
                                    });
                                },
                                click: function(e) {
                                    safeApply(leafletScope, function() {
                                        d3layer.selected = feature;
                                        $rootScope.$broadcast('leafletDirectiveMap.d3layerClick', d3layer.selected, e);
                                    });
                                }
                            });
                        };
                    ...
                    //leafletGeoJSON = L.geoJson(d3layer.data, d3layer.options);
                    leafletGeoJSON = new L.GeoJSON.d3(d3layer.data, {
                        core: core, //TODO
                        labels: true,
                        labelconfig: {
                            field: "name",
    ...
    var layerTypes = {
        ...
        d3layer:{
            ...
            createLayer: function(params) {
                ...
                return new L.GeoJSON.d3(params.options.data, params.options.options);
            }
        },
        geoJSON:{
            mustHaveUrl: true,
